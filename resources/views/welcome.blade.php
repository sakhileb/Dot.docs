...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="antialiased font-sans bg-white text-slate-900">
            ...
            <div class="max-w-7xl mx-auto px-6 lg:px-8">
                <div class="flex items-center justify-between h-16">
                    ...
                    <div class="hidden md:flex items-center gap-8">
                        <a href="#features" class="text-sm font-medium text-slate-600 hover:text-sky-600 transition-colors">Features</a>
                        <a href="#how-it-works" class="text-sm font-medium text-slate-600 hover:text-sky-600 transition-colors">How It Works</a>
                        <a href="#templates" class="text-sm font-medium text-slate-600 hover:text-sky-600 transition-colors">Templates</a>
                    </div>
                    <div class="flex items-center gap-3">
                        @if (Route::has('login'))
                            @auth
                                <a href="{{ url('/dashboard') }}" class="text-sm font-semibold text-sky-600 hover:text-sky-700 transition-colors">Dashboard &rarr;</a>
                            @else
                                <a href="{{ route('login') }}" class="text-sm font-medium text-slate-600 hover:text-slate-900 transition-colors">Log in</a>
                                @if (Route::has('register'))
                                    <a href="{{ route('register') }}" class="text-sm font-semibold px-4 py-2 bg-sky-600 hover:bg-sky-700 text-white rounded-lg transition-colors shadow-sm">Get Started</a>
                                @endif
                            @endauth
                        @endif
                    </div>
        ...
        <!-- ===================== HERO ===================== -->
        <section class="relative overflow-hidden bg-gradient-to-br from-amber-50 via-white to-sky-50 pt-20 pb-32">
            <div class="absolute inset-0 overflow-hidden pointer-events-none" aria-hidden="true">
                <div class="absolute -top-40 -right-40 w-96 h-96 bg-sky-200 rounded-full opacity-20 blur-3xl"></div>
                <div class="absolute -bottom-20 -left-20 w-80 h-80 bg-amber-200 rounded-full opacity-30 blur-3xl"></div>
            </div>

            <div class="relative max-w-5xl mx-auto px-6 text-center">
                <!-- Badge pill -->
                <div class="inline-flex items-center gap-2 bg-white border border-amber-200 text-amber-700 text-xs font-bold px-4 py-1.5 rounded-full mb-8 shadow-sm">
                    <span aria-hidden="true">&#10024;</span>
                    AI-Powered &middot; Real-time &middot; Offline-Ready
                </div>

                <!-- Headline -->
                <h1 class="text-5xl sm:text-6xl lg:text-7xl font-extrabold tracking-tight leading-tight mb-6 text-slate-900">
                    Write Smarter.<br>
                    <span class="bg-gradient-to-r from-amber-500 to-sky-500 bg-clip-text text-transparent">Collaborate Faster.</span>
                </h1>

                <!-- Subheadline -->
                <p class="max-w-2xl mx-auto text-lg sm:text-xl text-slate-600 leading-relaxed mb-10">
                    Dot.doc combines AI-assisted writing, real-time collaboration, smart templates,
                    and offline support &mdash; all in one beautifully crafted document platform.
                </p>

                <!-- CTAs -->
                <div class="flex flex-col sm:flex-row items-center justify-center gap-4 mb-16">
                    @if (Route::has('register'))
                        <a href="{{ route('register') }}" class="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-8 py-3.5 bg-sky-600 hover:bg-sky-700 text-white font-semibold rounded-xl shadow-md hover:shadow-lg transition-all text-lg">
                            Start Writing Free
                    ...
                    @endif
                    <a href="#features" class="w-full sm:w-auto inline-flex items-center justify-center px-8 py-3.5 bg-white border-2 border-sky-200 hover:border-sky-400 text-sky-700 font-semibold rounded-xl transition-all text-lg">
                        Explore Features
                    </a>
                </div>
                ...
                <div class="relative mx-auto max-w-4xl">
                    <div class="bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden">
                        ...
                        <div class="flex items-center gap-2 px-4 py-3 bg-slate-50 border-b border-slate-200">
                            ...
                            <div class="flex-1 mx-3 bg-white rounded border border-slate-200 px-3 py-1 text-xs text-slate-400 text-left">Q4 Project Proposal.doc</div>
                            <div class="flex items-center gap-1">
                                <div class="w-6 h-6 rounded-full bg-sky-500 text-white text-xs flex items-center justify-center font-bold">A</div>
                                <div class="w-6 h-6 rounded-full bg-amber-500 text-white text-xs flex items-center justify-center font-bold">B</div>
                                <div class="w-6 h-6 rounded-full bg-emerald-500 text-white text-xs flex items-center justify-center font-bold">C</div>
                            </div>
                        </div>

                        <!-- Toolbar -->
                        <div class="flex items-center gap-1 px-4 py-2 border-b border-slate-100 bg-white text-xs text-slate-500 overflow-x-auto whitespace-nowrap">
                            <span class="px-2 py-0.5 font-bold hover:bg-slate-100 rounded cursor-default select-none">B</span>
                            <span class="px-2 py-0.5 italic hover:bg-slate-100 rounded cursor-default select-none">I</span>
                            <span class="px-2 py-0.5 underline hover:bg-slate-100 rounded cursor-default select-none">U</span>
                            <span class="w-px h-4 bg-slate-200 mx-1 inline-block"></span>
                            <span class="px-2 py-0.5 hover:bg-slate-100 rounded cursor-default select-none">H1</span>
                            <span class="px-2 py-0.5 hover:bg-slate-100 rounded cursor-default select-none">H2</span>
                            <span class="w-px h-4 bg-slate-200 mx-1 inline-block"></span>
                            <span class="px-2 py-0.5 hover:bg-slate-100 rounded cursor-default select-none">&#8801;</span>
                            <span class="ml-auto inline-flex items-center gap-1 px-2.5 py-1 bg-amber-50 border border-amber-200 text-amber-700 rounded-md font-semibold cursor-default select-none">
                                &#10024; AI Assist
                            </span>
                        </div>
                        ...
                        <div class="p-8 text-left">
                            ...
                            <p class="text-slate-600 mb-4 leading-relaxed">This document outlines the key objectives and milestones for our upcoming product launch. The strategy focuses on three core pillars: <span class="text-sky-600 font-medium">user experience</span>, <span class="text-amber-600 font-medium">scalability</span>, and <span class="text-emerald-600 font-medium">team alignment</span>.</p>
                            <div class="inline-flex items-start gap-2 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 text-sm text-amber-800">
                                <span class="text-base mt-0.5" aria-hidden="true">&#10024;</span>
                                <span><strong>AI suggestion:</strong> Add a budget breakdown section to strengthen this proposal.</span>
                            </div>
                        </div>
                    ...
                    <div class="absolute -right-4 top-1/3 bg-white rounded-xl shadow-lg border border-slate-200 px-3 py-2 text-xs hidden lg:block" aria-hidden="true">
                    ...
                    <div class="absolute -left-4 bottom-1/4 bg-white rounded-xl shadow-lg border border-slate-200 px-3 py-2 text-xs hidden lg:block" aria-hidden="true">
        ...
        <section class="bg-white border-y border-slate-100 py-14">
            <div class="max-w-5xl mx-auto px-6">
                <div class="grid grid-cols-2 lg:grid-cols-4 gap-10 text-center">
                    <div>
                        <div class="text-4xl font-extrabold text-slate-900">&infin;</div>
                        <div class="text-sm text-slate-500 mt-2">Documents &amp; revisions</div>
                    </div>
                    <div>
                        <div class="text-4xl font-extrabold bg-gradient-to-r from-amber-500 to-sky-500 bg-clip-text text-transparent">AI</div>
                        <div class="text-sm text-slate-500 mt-2">Powered writing assistant</div>
                    </div>
                    <div>
                        <div class="text-4xl font-extrabold text-slate-900">RT</div>
                        <div class="text-sm text-slate-500 mt-2">Real-time collaboration</div>
                    </div>
                    <div>
                        <div class="text-4xl font-extrabold text-slate-900">100%</div>
                        <div class="text-sm text-slate-500 mt-2">Offline capable</div>
                    </div>
                </div>
            </div>
        </section>
        ...
        <section id="features" class="bg-slate-50 py-24">
            <div class="max-w-6xl mx-auto px-6">
                <div class="text-center mb-16">
                    <span class="text-xs font-bold uppercase tracking-widest text-sky-500">Platform Features</span>
                ...
                <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div class="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm hover:shadow-md transition-shadow">
                        <div class="w-11 h-11 bg-amber-100 rounded-xl flex items-center justify-center text-2xl mb-4" aria-hidden="true">&#10024;</div>
                        <h3 class="text-lg font-bold text-slate-900 mb-2">AI Writing Assistant</h3>
                        <p class="text-slate-500 text-sm leading-relaxed">Slash commands, smart completions, tone adjustments, and custom AI prompts &mdash; your writing gets smarter with every keystroke.</p>
                    </div>

                    <div class="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm hover:shadow-md transition-shadow">
                        <div class="w-11 h-11 bg-sky-100 rounded-xl flex items-center justify-center text-2xl mb-4" aria-hidden="true">&#128101;</div>
                        <h3 class="text-lg font-bold text-slate-900 mb-2">Real-time Collaboration</h3>
                        <p class="text-slate-500 text-sm leading-relaxed">See who&rsquo;s editing, leave inline comments, and track every change. Your whole team, in sync, in real time.</p>
                    </div>

                    <div class="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm hover:shadow-md transition-shadow">
                        <div class="w-11 h-11 bg-emerald-100 rounded-xl flex items-center justify-center text-2xl mb-4" aria-hidden="true">&#128196;</div>
                        <h3 class="text-lg font-bold text-slate-900 mb-2">Smart Templates</h3>
                        <p class="text-slate-500 text-sm leading-relaxed">Start fast with professional templates for proposals, reports, meeting notes, and more &mdash; or save your own.</p>
                    </div>

                    <div class="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm hover:shadow-md transition-shadow">
                        <div class="w-11 h-11 bg-violet-100 rounded-xl flex items-center justify-center text-2xl mb-4" aria-hidden="true">&#128336;</div>
                        <h3 class="text-lg font-bold text-slate-900 mb-2">Version History</h3>
                        <p class="text-slate-500 text-sm leading-relaxed">Every save is a snapshot. Browse, compare, and restore earlier versions of any document at any time.</p>
                    </div>

                    <div class="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm hover:shadow-md transition-shadow">
                        <div class="w-11 h-11 bg-orange-100 rounded-xl flex items-center justify-center text-2xl mb-4" aria-hidden="true">&#128244;</div>
                        <h3 class="text-lg font-bold text-slate-900 mb-2">Offline Mode</h3>
                        <p class="text-slate-500 text-sm leading-relaxed">Internet out? Keep writing. Dot.doc caches your work locally and syncs automatically when you reconnect.</p>
                    </div>

                    <div class="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm hover:shadow-md transition-shadow">
                        <div class="w-11 h-11 bg-rose-100 rounded-xl flex items-center justify-center text-2xl mb-4" aria-hidden="true">&#128228;</div>
                        <h3 class="text-lg font-bold text-slate-900 mb-2">Export &amp; Share</h3>
                        <p class="text-slate-500 text-sm leading-relaxed">Export to PDF or Word in one click. Share via link with view-only or edit permissions &mdash; no account required.</p>
                    </div>
        ...
        <section id="how-it-works" class="bg-white py-24">
            <div class="max-w-4xl mx-auto px-6">
                <div class="text-center mb-16">
                    <span class="text-xs font-bold uppercase tracking-widest text-amber-500">How it works</span>
                    <h2 class="mt-3 text-4xl font-extrabold text-slate-900">Up and writing in minutes</h2>
                </div>
                <div class="grid md:grid-cols-3 gap-10">
                    <div class="text-center">
                        <div class="w-14 h-14 bg-sky-600 text-white rounded-2xl flex items-center justify-center text-2xl font-extrabold mx-auto mb-5 shadow-md">1</div>
                        <h3 class="text-lg font-bold text-slate-900 mb-2">Create your workspace</h3>
                        <p class="text-slate-500 text-sm leading-relaxed">Sign up, create a team, and invite collaborators in under two minutes.</p>
                    </div>
                    <div class="text-center">
                        <div class="w-14 h-14 bg-amber-500 text-white rounded-2xl flex items-center justify-center text-2xl font-extrabold mx-auto mb-5 shadow-md">2</div>
                        <h3 class="text-lg font-bold text-slate-900 mb-2">Start from a template</h3>
                        <p class="text-slate-500 text-sm leading-relaxed">Pick a template or start blank. Let AI help you with the heavy lifting from the first word.</p>
                    </div>
                    <div class="text-center">
                        <div class="w-14 h-14 bg-emerald-600 text-white rounded-2xl flex items-center justify-center text-2xl font-extrabold mx-auto mb-5 shadow-md">3</div>
                        <h3 class="text-lg font-bold text-slate-900 mb-2">Collaborate &amp; ship</h3>
        ...
        <!-- ===================== CTA BANNER ===================== -->
        <section class="bg-gradient-to-r from-amber-500 to-sky-600 py-20">
            <div class="max-w-3xl mx-auto px-6 text-center">
                <h2 class="text-4xl font-extrabold text-white mb-4">Ready to write smarter?</h2>
                <p class="text-lg mb-10" style="color: rgba(255,255,255,0.85);">Join teams already creating better documents with Dot.doc.</p>
                <div class="flex flex-col sm:flex-row items-center justify-center gap-4">
                    @if (Route::has('register'))
                        <a href="{{ route('register') }}" class="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-8 py-3.5 bg-white hover:bg-slate-50 text-sky-700 font-semibold rounded-xl shadow-md hover:shadow-lg transition-all text-lg">
                            Get Started Free
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7l5 5m0 0l-5 5m5-5H6"/>
                            </svg>
                    ...
                    @if (Route::has('login'))
                        @guest
                            <a href="{{ route('login') }}" class="w-full sm:w-auto inline-flex items-center justify-center px-8 py-3.5 border-2 text-white font-semibold rounded-xl transition-all text-lg" style="border-color: rgba(255,255,255,0.5);" onmouseover="this.style.borderColor='rgba(255,255,255,1)'" onmouseout="this.style.borderColor='rgba(255,255,255,0.5)'">
        ...
        <footer class="bg-slate-900 text-slate-400 py-12">
            <div class="max-w-6xl mx-auto px-6">
                <div class="flex flex-col md:flex-row items-center justify-between gap-8">
                    <div class="flex items-center gap-3">
                        <img src="/dot_doc.png" alt="Dot.doc" class="h-8 w-auto opacity-90">
                        <span class="text-slate-500 text-sm">Write Smarter. Collaborate Faster.</span>
                    </div>
                    ...
                    <p class="text-xs text-slate-600">&copy; {{ date('Y') }} Dot.doc. All rights reserved.</p>
                </div>
            </div>
        </footer>
